...
public final class OKHTTP {
    private static Logger logger = LogManager.getLogger(OKHTTP.class);
    ...
    public enum ContentType {
        ...
        */
        ...
        public MediaType type(String charsetOrBoundary) {
            ...
            switch (this) {
                case form:{ return MediaType.parse("application/x-www-form-urlencoded; charset=" + charset); }
                case data:{ return MediaType.parse("multipart/form-data; boundary=" + boundary); }
                case json:{ return MediaType.parse("application/json; charset=" + charset); }
                case xml:{ return MediaType.parse("text/xml" + charset); }
                case text:{ return MediaType.parse("text/plain" + charset); }
                default: { return MediaType.parse("application/x-www-form-urlencoded; charset=" + charset); }
            }
        }
    }
    ...
    private static OkHttpClient getHttpClient() {
        ...
        builder.connectTimeout(30*1000, TimeUnit.MILLISECONDS);//默认30秒
        builder.readTimeout(30*1000, TimeUnit.MILLISECONDS);//默认30秒
        builder.writeTimeout(120000, TimeUnit.MILLISECONDS);

        //是否验证https证书，测试环境请设置false，生产环境建议优先尝试true，不行再false
        builder.sslSocketFactory(new BaseHttpSSLSocketFactory());
        builder.hostnameVerifier(new TrustAnyHostnameVerifier());//解决由于服务器证书问题导致HTTPS无法访问的情况
    ...
    public static String post(String url, String content, Map<String,String> header) throws Exception {
        try {
            RequestBody body = RequestBody.create(ContentType.text.type(), content);
            Request.Builder builder = new Request.Builder()
                    .url(url)
                    .post(body);
            if (header != null) {
                writeRequestHeader(builder,header);
            }
...
//            e.printStackTrace();
    ...
    public static String postForm(String url, String content, Map<String,String> header) throws Exception {
        try {
            RequestBody body = RequestBody.create(ContentType.form.type(), content);
            Request.Builder builder = new Request.Builder()
                    .url(url)
                    .post(body);
            if (header != null) {
...
//            e.printStackTrace();
    ...
    public static String post(String url, Map<String, String> params, Map<String,String> header, int connectTimeout, int readTimeout) throws Exception {
        try {
            RequestBody body = RequestBody.create(ContentType.form.type(), getFormParamsString(params,ESBConsts.UTF8_STR));
            Request.Builder builder = new Request.Builder()
                    .url(url)
                    .post(body);
            if (header != null) {
    ...
    public static String postJSON(String url, Object object, Map<String,String> header, int connectTimeout, int readTimeout) throws Exception {
        try {
            ...
            if (object instanceof String) {
                ...
                json = JSON.toJSONString(object, ESBConsts.FASTJSON_SERIALIZER_FEATURES);
            }
            RequestBody body = RequestBody.create(ContentType.json.type(), json);
            Request.Builder builder = new Request.Builder()
                    .url(url)
                    .post(body);
            if (header != null) {
...
//            e.printStackTrace();
    ...
    public static String postData(String url, byte[] data, String fileName, Map<String,String> header, int connectTimeout, int readTimeout) throws Exception {
        try {
            ...
            MediaType type = MediaType.parse(FileContentType.getContentTypeForFileName(fileName));
            RequestBody body = RequestBody.create(type,new File(path));
            Request.Builder builder = new Request.Builder()
                    .url(url)
                    .post(body);
            if (header != null) {
...
//            e.printStackTrace();
    ...
    public static String get(String url, Map<String, String> params, Map<String,String> header, Map<String,String> cookie, int connectTimeout, int readTimeout) throws Exception {
        try {
            ...
            Request.Builder builder = new Request.Builder()
                    .url(u.getProtocol()+ "://" + u.getHost() + (u.getPort() > 0 ? ":" + u.getPort() : "") + u.getPath() + "?" + q);
            if (header != null) {
                writeRequestHeader(builder,header);
            }
            if (cookie != null) {
                writeCookie(builder,header);
            }
            Request request = builder.build();
            Response response = getHttpClient().newCall(request).execute();
            String result = response.body().string();
    ...
    private static void writeRequestHeader(final Request.Builder builder, Map<String,String> header)
            throws Exception {
    ...
    private static void writeCookie(final Request.Builder requestBuilder, Map<String,String> cookie)
            throws Exception {
        ...
        if (builder.length() > 0) {
            ...
            writeRequestHeader(requestBuilder, ck);
        }
    }
